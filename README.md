...
- Mini Games (🎮, in the Menu) opens `minigames.html`, a separate bonus page with three small arcade games — each with its own best score saved locally, completely separate from Jumble's own stats and leaderboards. Snake has a pause button (or Space) with a paused overlay; Memory Match has an Easy 4×4 / Hard 6×4 difficulty toggle, each tracking its own best; 2048 has a one-step Undo. All three celebrate a new best score with a confetti burst, and Esc returns to the arcade hub from any game
