...
- Sprint Mode (⏱️, in Settings) caps a run at 30 placements — score as high as you can before time's up. Has its own named leaderboard, separate from Classic play
...
- Top-10 named high score leaderboard (🏆) with tabs for Classic (split by board size), Sprint, Time Attack, and Survival — each mode's runs are only ever compared against its own kind — plus session stats (games played, lines cleared, best combo, daily play streak, today's best score, total time played)
