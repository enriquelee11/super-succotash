...
- Settings menu (⚙️) with sound/haptics toggles, a two-tap "reset all progress" option, and a dedicated Themes page (🎨, its own screen off Settings) with 115 preset color themes — tap one to preview its name and a one-line description, right below the grid — plus a fully custom theme you build from 4 picked colors
...
- Time Attack (⏰, in Settings) gives you a countdown — 1, 2, or 3 minutes, your pick — to score as high as possible, with a pause button (⏸️, or the P key) that freezes the clock mid-run — it also auto-pauses if you switch tabs or apps, so the clock never burns down while you're away. Has its own named leaderboard
...
- Sprint, Time Attack, and Survival are mutually exclusive — turning one on turns the other two off, since each is its own fixed-format run with its own leaderboard
...
- Mini Games (🎮, in the Menu) opens `minigames.html`, a separate bonus page with three small arcade games — each with its own best score saved locally, completely separate from Jumble's own stats and leaderboards. All three celebrate a new best score with a confetti burst, show pop-up toasts for in-game events, and Esc returns to the arcade hub from any game
  - Snake has a pause button (or Space) with a paused overlay, occasional golden bonus apples worth extra points (that despawn if you dawdle), and obstacles that start appearing on the board as your score climbs
  - Memory Match briefly shows every card face-up to memorize before flipping them down, has an Easy 4×4 / Hard 6×4 difficulty toggle (each tracking its own best), and rates your solve 1-3 stars based on how few moves it took
  - 2048 has a one-step Undo and pays out a growing bonus (added straight to your score) the first time you build a 128, 256, 512, 1024, or 2048 tile
...
- Weekly Challenge (📆, in the Menu) is a tougher numbered puzzle that stays the same for everyone all week, with Hard Mode's no-Undo/no-Reroll rules always on, plus one rotating weekly curse (also the same for everyone that week, previewed on the Menu before you start): Iron Tide (a cell locks in permanently every 6 placements), Heavy Hand (only medium/large pieces are dealt — no easy singles, dominoes, or triples), Blackout (no line-clear preview while dragging), or Frozen Start (the board starts 15% pre-filled). Has its own leaderboard tab, best score, and completion streak
...
- Sound effects (picking up a piece, placements — pitched lower for bigger pieces, a soft buzz for a failed drop, a fresh-hand chime when your tray refills, line clears, Perfect Clears, combos, a distinct sparkly chime for Lucky Pieces, Undo, Reroll, a shuffle whoosh for Zen/Practice reshuffles, a rumble when Survival's tide rises or the Weekly Challenge's Iron Tide curse locks in a cell, a two-note chime for badge unlocks, a bigger fanfare for rank-ups, a rising sweep for a new #1 score, and game over) and haptic feedback on mobile, with a mute toggle (🔊/🔇)
...
- Warm pastel visual theme by default, with chunky, blocky display type, 115 alternate preset themes (including an Accessible theme built on the colorblind-safe Okabe-Ito palette), and a build-your-own custom theme
